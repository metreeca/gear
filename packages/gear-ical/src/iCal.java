...
package com.metreeca.flow.ical.formats;
 ...
 *
 * @see <a href="https://www.rfc-editor.org/rfc/rfc5545.html">RFC 5545 - Internet Calendaring and Scheduling Core
 *         Object Specification (iCalendar)</a>
 */
public final class iCal implements Format<Calendar> {
     ...
     *         optional if the {@code "Content-Type"} {@code message} header is not matched by {@link #MIME}
     */
    @Override public Optional<Calendar> decode(final Message<?> message) throws FormatException, UncheckedIOException {
        return Optional.of(message.type(MIME)).filter(MIME::equals).map(type -> {
            ...
            try ( final InputStream input=message.input().get() ) {
                ...
                throw new FormatException(Response.BAD_REQUEST, e.getMessage());
     ...
     *         already defined, and its raw {@linkplain Message#output(Consumer) output} configured to return the
     *         iCalendar {@code value}
     */
